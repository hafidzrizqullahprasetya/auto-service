...
export function StockMovementForm({
  ...
  const physicalItems = MOCK_ITEMS.filter((i) => i.category !== "Service");
        ...
        <div className="flex items-center justify-between">
          {!hasEnoughStock && (
            <p className="text-xs font-bold text-red-500">
              ⚠ Stok tidak cukup (tersedia: {selectedItem?.stock} {selectedItem?.unit})
            </p>
          )}
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <div className="max-h-48 overflow-y-auto rounded-lg border border-stroke dark:border-dark-3">
            {filtered.map((item) => (
              ...
              >
                <span>
                  <span className="font-mono text-xs text-dark-5">{item.sku}</span>
                  {" · "}
                  {item.name}
                </span>
            ...
            {filtered.length === 0 && (
              <p className="py-4 text-center text-sm text-dark-5">Item tidak ditemukan</p>
            )}
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-dark-5">
                  ...
                  <span
                    className={`font-bold ${
                      (selectedItem.stock ?? 0) <= (selectedItem.minimumStock ?? 0)
                        ? "text-red-500"
                        : "text-secondary"
                    }`}
                  >
              ...
              <Badge variant={selectedItem.type === "Mobil" ? "primary" : "info"} className="text-[10px]">
                {selectedItem.type}
              </Badge>
            </div>
            <p className="mt-1 text-xs font-bold text-dark-5">
              Harga Jual: Rp {formatNumber(selectedItem.price)} / {selectedItem.unit}
            </p>
          </div>
        )}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3">
            ...
            {selectedItem && (
              <span className="text-sm text-dark-5 font-medium">{selectedItem.unit}</span>
            )}
          </div>
        </div>

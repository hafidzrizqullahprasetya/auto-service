...
export function CreateTransactionForm({
  ...
  const addItemToCart = (item: Item) => {
    setCart((prev) => {
      ...
      return [
        ...prev,
        { item, isJasa: false, jasaName: "", jasaPrice: 0, quantity: 1 },
      ];
    });
    setItemSearch("");
  };
  ...
  const updateQty = (index: number, qty: number) => {
    setCart((prev) =>
      prev.map((ci, i) =>
        i === index ? { ...ci, quantity: Math.max(1, qty) } : ci,
      ),
    );
  };
      ...
      {step === 1 ? (
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
              ...
              {itemSearch && (
                <div className="max-h-40 overflow-y-auto rounded-lg border border-stroke dark:border-dark-3">
                  ...
                  {filteredItems.length === 0 && (
                    <p className="py-3 text-center text-sm text-dark-5">
                      Tidak ditemukan
                    </p>
                  )}
                </div>
              ...
              <div className="space-y-3 rounded-lg border border-stroke p-4 dark:border-dark-3">
                ...
                <div className="flex gap-2">
                  <ActionButton
                    variant="primary"
                    label="Tambah"
                    onClick={addJasaToCart}
                  />
                  <ActionButton
                    variant="ghost"
                    label="Batal"
                    onClick={() => setAddingJasa(false)}
                  />
                </div>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-bold text-dark dark:text-white">
                Catatan{" "}
                <span className="font-normal text-dark-5">(opsional)</span>
              </label>
              <textarea
                rows={2}
                value={notes}
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-dark-5">
                          ...
                          {ci.isJasa ? "jasa" : (ci.item?.unit ?? "pcs")}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => updateQty(idx, ci.quantity - 1)}
        ...
        <div className="mx-auto max-w-md space-y-5">
          ...
          <div className="rounded-xl border border-stroke bg-gray-1 p-4 dark:border-dark-3 dark:bg-dark-2">
            ...
            <div className="mt-2 flex items-center justify-between border-t border-stroke pt-2 dark:border-dark-3">
              <span className="text-sm font-bold text-dark dark:text-white">
                Total Tagihan:
              </span>
              ...
              {paidAmount > 0 && (
                <p className="text-xs text-dark-5">
                  ...
                  <span className="font-bold text-secondary">
                    Rp {formatNumber(Math.max(0, sisa))}
                  </span>
                </p>
              )}
            </div>
